...
const ChatContainer = ({selectedUser,setSelectedUser}) => {
  ...
  return selectedUser ? (
      ...
      <div className='flex flex-col h-[calc(100%_-_120px)] overflow-y-scroll p-3 pb-6'>
            {
                messagesDummyData.map((msg, index) => (
                    ...
                    ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse' }`}>
                        {
                            msg.image ? (
                                <img src = {msg.image} alt='img' className='max-w-[230px] border border-gray-700 rounded-lg 
                                overflow-hidden mb-8'/>
                            ):(
                                <p className={`p2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 
      ...
      {/* ---------------------------------------------input area ----------------------------------------- */}
        <div className='absolute bottom-0 left-0 right-0 p-3  flex items-center gap-3'>
            <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                ...
                <input type="file" id='image' accepts='image/png, image/jpeg' hidden/>
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      ...
      <p className='text-white font-medium text-lg mt-2'>Chat anytime, anywhere</p>
    </div>
  )
}
